...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#0f0f11] text-[#e0e0e0] font-sans selection:bg-[#00f2ff] selection:text-[#000]">
      ...
      {/* Header */}
      <nav className="relative z-10 flex justify-between items-center p-4 md:p-6 border-b border-[#222]">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-[#00f2ff] flex items-center justify-center rounded-sm shrink-0">
            <span className="text-[#000] font-black text-xl">A</span>
          </div>
          <span className="font-bold tracking-tighter text-lg md:text-xl text-white truncate">AINTLY // SAMPLE_02</span>
        </div>
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-6 pt-12 md:pt-24 pb-20 md:pb-32">
        <div className="flex flex-col lg:flex-row gap-12 md:gap-16 items-center">
          ...
          <div className="flex-1 space-y-6 md:space-y-8 text-center lg:text-left">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-4">
              <button className="w-full sm:w-auto bg-white text-black px-8 py-4 font-bold flex items-center justify-center gap-2 hover:bg-[#00f2ff] transition-all group">
                INITIATE PROTOCOL <ArrowRight size={20} className="group-hover:translate-x-1 transition-transform"/>
              </button>
              <button className="w-full sm:w-auto border border-zinc-700 px-8 py-4 font-bold flex items-center justify-center gap-2 hover:bg-zinc-800 transition-all">
          ...
          <div className="flex-1 w-full max-w-xl">
            <div className="bg-[#1a1a1e] border border-zinc-800 rounded-lg overflow-hidden shadow-2xl">
              <div className="flex items-center gap-2 px-4 py-2 bg-[#252529] border-b border-zinc-800">
                ...
                <span className="ml-4 font-mono text-[10px] md:text-xs text-zinc-500 uppercase tracking-widest truncate">aintly_core.sh</span>
              </div>
              <div className="p-4 md:p-6 font-mono text-xs md:text-sm space-y-2 text-[#00f2ff]/80 overflow-x-auto">
                <p className="whitespace-nowrap"><span className="text-zinc-600">$</span> brew install aintly-core</p>
                <p className="text-zinc-500 whitespace-nowrap">{" >> "}resolving dependencies...</p>
                <p className="text-zinc-500 whitespace-nowrap">{" >> "}configuring neural bridge...</p>
                <p className="whitespace-nowrap"><span className="text-zinc-600">$</span> aintly up --cluster=02</p>
                <p className="text-white bg-[#00f2ff]/20 px-2 inline-block whitespace-nowrap">DEPLOYMENT SUCCESSFUL [PID 9924]</p>
                <p className="text-zinc-400 whitespace-nowrap">ENDPOINT: sample2.aintly.com</p>
                <div className="mt-8 grid grid-cols-2 gap-3 md:gap-4">
                  <div className="border border-zinc-800 p-3 md:p-4 bg-zinc-900/50">
                    <Cpu className="mb-2 text-[#7000ff]" size={18} />
                    <div className="text-[10px] text-zinc-500">LOAD</div>
                    <div className="text-base md:text-lg font-bold text-white">4.2%</div>
                  </div>
                  <div className="border border-zinc-800 p-3 md:p-4 bg-zinc-900/50">
                    <Zap className="mb-2 text-[#00f2ff]" size={18} />
                    <div className="text-[10px] text-zinc-500">LATENCY</div>
                    <div className="text-base md:text-lg font-bold text-white">12ms</div>
                  </div>
                </div>
              </div>
            </div>
